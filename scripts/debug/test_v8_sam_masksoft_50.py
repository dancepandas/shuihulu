"""
用 V8 YOLO + SAM(mask prompt + 软约束) 跑 50 张图
软约束: SAM mask 与 YOLO mask 膨胀版取交集
输出: temp_v8_sam_masksoft_50/
"""
import random
...
def main():
    ...
    for img_path in sorted(sample):
        ...
        for j in target_idx:
            ...
            # 软约束: YOLO mask 膨胀 30px，SAM mask 与之取交集
            if j < len(xy) and len(xy[j]) >= 3:
        ...
        cv2.putText(canvas, f"mask+soft n={len(masks)}", (10, 25),
