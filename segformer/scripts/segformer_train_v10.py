"""SegFormer B2 语义分割 — 水葫芦 v10 (5类新标签体系)
数据: datasets/hyacinth_ls_v10 (LS project 10, 5类: 0=water 1=water_hyacinth 2=hard_structure 3=shore_vegetation 4=other_aquatic_vegetation)
主干: weights/segformer_b2_seg (NVIDIA ADE20K 语义分割预训练, 非 ImageNet 分类头)
策略: 完整 LWH-Seg 两段式 (冻结编码器20ep -> 解冻联合微调60ep) + 类别加权CE + Dice 联合损失
输入: 640x640, batch=1 + 梯度累积(累积4步=等效batch4), RTX 2060 5GB 安全
"""
...
DATASET_DIR = os.path.join(ROOT, "datasets", "hyacinth_ls_v10")
OUTPUT_DIR = os.path.join(ROOT, "runs", "segformer", "segformer_b2_ls_v10")
MODEL_PATH = os.path.join(ROOT, "weights", "segformer_b2_seg")  # 语义分割预训练
HF_MODEL_ID = "nvidia/segformer-b2-finetuned-ade-512-512"       # 云端/无本地权重时回退到 HF 下载

IMAGE_SIZE = 640
BATCH_SIZE = 1            # 显存最小；配合累积等效大 batch
GRAD_ACCUM = 4            # 累积 4 步 = 等效 batch 4
NUM_EPOCHS = 80
PHASE1_EPOCHS = 20        # 第一阶段冻结编码器
LR, WD = 6e-5, 0.01
NUM_CLASSES = 5
LOSS_ALPHA, LOSS_BETA = 1.0, 0.5   # 联合损失权重 (WCE + Dice)
DICE_EPS = 1.0
...
def compute_class_weights(split="train"):
    ...
    w = inv / inv.sum() * NUM_CLASSES   # 归一化，均值≈1
...
def wce_dice_loss(logits, labels, weights, alpha=1.0, beta=0.5, eps=1.0):
    """logits: (B, C, H, W), labels: (B, H, W), weights: (C,)"""
    B, C, H, W = logits.shape
    # 类别加权 CE (log_softmax + nll, 带 weight)
    logp = F.log_softmax(logits, dim=1)
    loss_ce = F.nll_loss(logp, labels, weight=weights, ignore_index=255)
    # Dice 损失 (逐类别, 在 logits 分辨率上, 与标签对齐)
    pred = logp.exp()   # softmax 概率
...
def train():
    ...
    # 语义分割预训练权重 (ADE20K), 换 head 为 5 类
    # 优先用本地 weights/segformer_b2_seg; 不存在(云端)则自动从 HF 下载
    model_path = MODEL_PATH
    if not os.path.isdir(MODEL_PATH):
        print(f"[weights] local not found: {MODEL_PATH}")
        print(f"[weights] downloading from HF: {HF_MODEL_ID}")
    ...
    # 两段式: 第一阶段冻结 encoder, 只训 decoder; 第二阶段全部
    def set_frozen(frozen):
        for n, p in model.named_parameters():
            if n.startswith("segformer.encoder."):
                p.requires_grad = not frozen
            else:
                p.requires_grad = True
        n_train = sum(1 for p in model.parameters() if p.requires_grad)
        n_tot = sum(1 for p in model.parameters())
        print(f"  [phase] {'FREEZE encoder (decoder only)' if frozen else 'UNFREEZE all'} "
    ...
    for ep in range(NUM_EPOCHS):
        set_frozen(ep < PHASE1_EPOCHS)   # 第 1-20 冻结, 21-80 解冻
        ...
        for b in tqdm(train_ld, desc=f"E{ep+1}/{NUM_EPOCHS} train"):
            ...
            # 对齐 logits 分辨率到 label (albumentations Resize 是同一尺寸, 但 SegFormer 输出 1/4)
            logits = out.logits
        ...
        phase = "FROZEN" if ep < PHASE1_EPOCHS else "UNFREEZED"
        print(f"E{ep+1} [{phase}]: loss={total/n_batches:.4f}  mIoU={m['mIoU']:.4f}  "
