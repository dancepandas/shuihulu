#!/usr/bin/env python
"""YOLO 检测路线 Web 推理服务 — 纯 YOLO / YOLO+GLI 融合 / YOLO+SAM 三模式
(自根目录 app.py 拆分; SegFormer 两模式见 segformer/app.py)

启动: python yolo_sam/app.py  →  http://localhost:5000
"""
...
<title>模型识别结果测试 — YOLO 检测路线</title>
<style>
...
.btn-group label {
  ...
  background: #fff; color: #555; border-right: 1px solid #d0d5dd; transition: background .15s;
}
...
<div class="header">
  <h1>模型识别结果测试 — YOLO 检测路线</h1>
  <p>上传无人机影像，对比 纯 YOLO / YOLO+GLI / YOLO+SAM 三种方法检测效果</p>
</div>
...
const ALL_METHODS = ['yolo', 'gli', 'sam'];
...
def predict():
    try:
        ...
        if mode == "yolo":
            ...
            return jsonify({"error": f"未知模型模式: {mode} (本服务仅 yolo/gli/sam)"}), 400
